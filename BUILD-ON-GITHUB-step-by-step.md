...
- **Add an introducer on the fly:** when creating a **Pipeline** opportunity **or a Client**, the introducer/Source dropdown includes **➕ Add a new introducer…** — fill in a few fields and it's saved into your Introducers automatically and linked, without leaving the form.
- **Alphabetical by default:** Introducers are listed by **organisation A→Z, then contact name**, and Clients by **name A→Z**, so anyone you add drops straight into the right place (no more hunting at the bottom). Click any column header to re-sort.
...
- **Meetings & Calendar:** the **Meetings** tab lets you schedule in-person/virtual/phone meetings with any introducer or client (date, time, duration, location or video link, agenda). When the time passes, the meeting moves to **“Needs your confirmation”** (also on **Today**): click **✓ Held** to record what was discussed — it's logged straight into that contact's interaction history and resets their catch-in clock — or **✗ Didn't happen** (then reschedule). Your **next 7 days** of meetings show on the dashboard, with one-tap **Join** links for virtual calls.
- **Add meetings to your iPhone/Mac Calendar:** first, in **Settings → Calendar & invites**, enter your **calendar invite email** (the personal Gmail you want events tagged with) — it's stored encrypted in your vault, never on GitHub. Then, when you save a new future meeting:
   - **On iPhone/iPad (any browser, incl. Chrome):** the app opens **Google Calendar** pre-filled — just tap **Save**. It lands in your Google Calendar and syncs to the iPhone **Calendar** app (via your Google account). *(iOS browsers can't reliably download an .ics, which is why the Google route is used on the phone.)*
   - **On a computer:** it downloads an **.ics** file that opens in Apple Calendar / Outlook.
   - Each meeting also has explicit **📅 Add to calendar**, **Google**, and **.ics** buttons for manual adds or after a reschedule; auto-add can be switched off in Settings.
   - *A secure static page can't silently send server emails, so this is the standard, private way to get events onto your devices.*
